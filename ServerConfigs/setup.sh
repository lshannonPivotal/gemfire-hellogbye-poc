...
if [[ ("$option" -eq "4") ]]; then
	ssh -i gemfire.pem ubuntu@$LOCATOR_SERVER_1 $DATA_DIRECTORY/deletedata.sh $total
	ssh -i gemfire.pem ubuntu@$LOCATOR_SERVER_1 $DATA_DIRECTORY/listdata.sh
	ssh -i gemfire.pem ubuntu@$LOCATOR_SERVER_2 $DATA_DIRECTORY/deletedata.sh $total
	ssh -i gemfire.pem ubuntu@$LOCATOR_SERVER_2 $DATA_DIRECTORY/listdata.sh
