...
gfsh -e "start server --name=$SERVER_NAME --use-cluster-configuration=false --classpath=$CLASSPATH --bind-address=$HOSTNAME --dir=$SERVER_DIR_LOCATION/$SERVER_NAME --locators=$LOCATOR_1_IP[$LOCATOR_PORT],$LOCATOR_2_IP[$LOCATOR_PORT] --properties-file=$CONF_DIR/gemfire.properties --rebalance=true --J=-Xms31g --J=-Xmx31g --cache-xml-file=$CONF_DIR/cache.xml"
